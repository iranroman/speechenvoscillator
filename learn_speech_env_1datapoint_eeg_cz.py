...
def butter_bandpass_filter(data, lowcut, highcut, fs, order=5):
    ...
    y = filtfilt(b, a, data)
    return y
all_mats = os.listdir('data')
all_envelopes = []
for mat in all_mats:
    data = scipy.io.loadmat('data/'+mat)
    all_envelopes.extend(list(np.squeeze(data['S'])))
eeg = []
for mat in all_mats:
    data = scipy.io.loadmat('data/'+mat)
    eeg.extend([butter_bandpass_filter(x[:,47],3.0,8.0,fs) for x in data['R_zscore'][0]])
...
nchannels = 1 
stim_values = all_envelopes[[0],:-lag,np.newaxis]
...
for e in range(num_epochs):
    ...
    curr_corr = np.mean(correlation_numpy(eeg[[0],lag+1:-500],cleaned.numpy()[:,:-500]))
    print(curr_corr)
    if curr_corr > best_corr:
        ...
        plt.grid()
        plt.plot(GrFNN.time[:-1],np.squeeze(eeg[[0],lag+1:]))
