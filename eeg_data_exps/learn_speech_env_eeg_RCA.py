...
data = scipy.io.loadmat('data/RCAanalysis_output.mat')['dataOut']
eeg = np.mean(data,axis=-1)
eeg = np.reshape(eeg[:35340,0],(12,-1))

data = scipy.io.loadmat('data/SRmats_12segments/sub200_SR_12equal_segments.mat')
all_envelopes = np.reshape(data['wholeS'][0,:35340],(12,-1))
...
all_envelopes = [butter_lowpass_filter(np.squeeze(np.array(env)),cutoff,fs) for env in all_envelopes]
...
mask = np.ma.masked_where(all_envelopes[:,2:] != 0.0, all_envelopes[:,2:]).mask.astype(np.float32)
...
#eeg = np.array([np.pad(butter_bandpass_filter(np.squeeze(env),3.0,8.0,fs),(0,max_len-len(env))) for env in eeg])
eeg = (eeg-np.expand_dims(np.mean(eeg, axis=1),-1))/np.expand_dims(np.std(eeg, axis=1),-1)/4

#idxs = []
#for i in range(len(eeg)):
#    #print(pearsonr(eeg[i,lag:],all_envelopes[i,:-lag])[0])
#    if pearsonr(eeg[i,lag:],all_envelopes[i,:-lag])[0]>0.0:
#        #print(i)
#        idxs.append(i)
#print(np.mean([pearsonr(i,j) for i,j in zip(eeg[idxs,lag:],all_envelopes[idxs,:-lag])]))
#print(len(idxs))
#input()
...
nchannels = 1 
stim_values = all_envelopes[:,:-lag-1,np.newaxis]
...
#scat_mat = np.load('W.npy')
#scat_b = np.load('b.npy')
eeg_hb = np.cos(np.angle(hilbert(eeg, axis=1)))
clean_values = tf.convert_to_tensor(eeg_hb[:,lag+1:-1,np.newaxis], dtype=tf.float32)
...
CE = tf.losses.CategoricalCrossentropy(from_logits=True)
def train_step(optim, target, mask, time, layers_state, layers_alpha, layers_beta1, layers_beta2, layers_delta, layers_cz, layers_cw, layers_cr, layers_w0, layers_epsilon, zfun, stim_values, dtype):
    with tf.GradientTape() as tape:
        ...
        l_output_r = tf.transpose(l_output_r,(1,0,2))
        l_output_i = tf.transpose(l_output_i,(1,0,2))
...
for e in range(num_epochs):
    ...
    curr_corr = np.mean(correlation_numpy(eeg[:,lag+1:-501],cleaned.numpy()[:,:-500]))
    print(curr_corr)
    if curr_corr > best_corr:
        print("New best corr coeff found!")
        best_corr = curr_corr
        plt.rcParams["figure.figsize"] = (20,10)
        plt.grid()
        plt.plot(GrFNN.time[:-1],np.squeeze(eeg[[1],lag+1:-1]))
        plt.plot(GrFNN.time[:-1],np.squeeze(cleaned[1]))
        plt.plot(GrFNN.time[:-1],np.squeeze(frq[1]), '--')
        plt.title('corr: '+"{:.3f}".format(best_corr)+', loss: '+"{:.3f}".format(loss.numpy())+'   '+', '.join([k+"{:.3f}".format(v.numpy()) for k, v in var_list_old.items()]))
